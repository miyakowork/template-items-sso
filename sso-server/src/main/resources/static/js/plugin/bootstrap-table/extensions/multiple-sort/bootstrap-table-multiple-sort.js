...
(function ($) {
    ...
    var showSortModal = function (that) {
        ...
        if (!$(_id).hasClass("modal")) {
            var sModal = '  <div class="modal fade" id="' + _id + '" tabindex="-1" role="dialog" aria-labelledby="' + _id + 'Label" >';
            sModal += '         <div class="modal-dialog">';
            sModal += '             <div class="modal-content">';
            sModal += '                 <div class="modal-header">';
            sModal += '                     <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span >&times;</span></button>';
            ...
            sModal += '                     <button type="button" class="btn btn-primary" id="' + _id + '_onSort">' + that.options.formatSort() + '</button>';
            sModal += '                 </div>';
            sModal += '             </div>';
            sModal += '         </div>';
            sModal += '     </div>';
            ...
            that.$sortModal.off('click', '#' + _id + '_onSort').on('click', '#' + _id + '_onSort', function () {
                ...
                $.each($rows, function (index, row) {
                    ...
                    sorts += name.concat(",").concat(order).concat(";");
                })

                //此处的是用作传递参数用的，与下面的值一致，只不过方式 不同，此处为字符串拼接
                that.options.sortPriority4QueryParams = sorts.substring(0, sorts.length - 1);

                //此处是用作页面时排序显示用的，和上面的一致，只不过方式不同，此处为对象数组
                ...
                if (results.length > 0) {
                    ...
                    if (that.options.sidePagination === 'server') {
                        ...
                        that.options.queryParams = function (params) {
                            params.multiSort = that.options.sortPriority4QueryParams;
                            return params;
                        };
        ...
        for (var i = 0; i < headers.length; i++) {
            for (var c = 0; c < that.options.sortPriority.length; c++) {
                if ($(headers[i]).data('field') === that.options.sortPriority[c].sortName
                    || $(headers[i]).data('sortname') === that.options.sortPriority[c].sortName) {
