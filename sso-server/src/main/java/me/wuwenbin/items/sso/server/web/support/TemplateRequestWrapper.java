package me.wuwenbin.items.sso.server.web.support;
...
public class TemplateRequestWrapper extends HttpServletRequestWrapper {
    ...
    protected String filterParamStrings(String beforeFilter) {
        if (null == beforeFilter) {
            return null;
        }
        String afterFilter = beforeFilter;
        if (this.filterXSS) {
            afterFilter = Injection.stripSqlXSS(afterFilter);
        }
        if (this.filterSQL) {
            afterFilter = Injection.stripSqlInjection(afterFilter);
        }
        return afterFilter;
    }
